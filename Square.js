function Square(_x, _y, _difficulty, _squareType) {
   ...
   this.neighbors = function(board) {
      neighbors = [];
      if (this.x > 0 && (board[this.x - 1][this.y].squareType === PATH || board[this.x - 1][this.y].squareType == DOOR || board[this.x - 1][this.y].squareType == STAIRDOWN)) {
         neighbors.push(board[this.x - 1][this.y]);
      }
      if (this.x < numSquares - 1 && (board[this.x + 1][this.y].squareType === PATH || board[this.x + 1][this.y].squareType == DOOR || board[this.x + 1][this.y].squareType == STAIRDOWN)) {
         neighbors.push(board[this.x + 1][this.y]);
      }
      if (this.y > 0 && (board[this.x][this.y - 1].squareType === PATH || board[this.x][this.y - 1].squareType == DOOR || board[this.x][this.y - 1].squareType == STAIRDOWN)) {
         neighbors.push(board[this.x][this.y - 1]);
      }
      if (this.y < numSquares - 1 && (board[this.x][this.y + 1].squareType === PATH || board[this.x][this.y + 1].squareType == DOOR || board[this.x][this.y + 1].squareType == STAIRDOWN)) {
         neighbors.push(board[this.x][this.y + 1]);
      }
      return neighbors;
   };
